 ...
 *
 * This module is the wiring point named by E1-06's constraint note: when a
 * second interface version ships, the workflow store's verification
 * (`getPublishedVersion`) must select its rule set from the stored
 * `interface_version` through this registry (E1-S4), instead of the single
 * default preparer `createWorkflowDatabase` constructs. No storage change
 * is needed while v1 is the only supported version.
 */
