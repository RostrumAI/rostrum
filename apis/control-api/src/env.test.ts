...
describe("loadConfig", () => {
    ...
    test("production defaults logging to info", () => {
        expect(loadConfig({ NODE_ENV: "production" }, {}).logLevel).toBe("info");
    });

    test("an explicit LOG_LEVEL wins over the environment default", () => {
        expect(loadConfig({ NODE_ENV: "development", LOG_LEVEL: "warning" }, {}).logLevel).toBe(
            "warning",
        );
    });
    ...
    test("rejects values that fail the config schema", () => {
        ...
        expect(() => loadConfig({ NODE_ENV: "staging" }, {})).toThrow(/nodeEnv/);
    });
