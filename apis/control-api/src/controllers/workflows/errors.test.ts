...
describe("errorPayloadFor", () => {
    ...
    test("maps the service's parse failure to 400 carrying its findings", () => {
        const payload = errorPayloadFor(new WorkflowDocumentParseError(FINDINGS));
        expect(payload?.status).toBe(400);
        expect(payload?.code).toBe("invalid_workflow_input");
        expect(payload?.findings).toEqual(FINDINGS);
    });

    test("maps the service's identity conflict to 409 carrying its message", () => {
        const payload = errorPayloadFor(
            new WorkflowIdentityConflictError("The embedded id does not match"),
        );
        expect(payload?.status).toBe(409);
        expect(payload?.code).toBe("identity_conflict");
        expect(payload?.message).toBe("The embedded id does not match");
    });

    test("maps input the storage layer rejected to 400", () => {
        const payload = errorPayloadFor(new WorkflowInputError("'x' is not a valid workflow id"));
        expect(payload?.status).toBe(400);
        expect(payload?.code).toBe("invalid_workflow_input");
