...
- Severity: `critical` = a defect that breaks behavior or is exploitable; `high` = an obvious bug, or a weaker security weakness; `medium` = a convention broken at declaration or file scope; `low` = a line-level problem such as missing braces or a comment that restates its code; `informational` = a suggestion with no defect. `medium` and above are BLOCKING. Violating a rule is not by itself proof of the worst case: grade what the changed code actually does, and when unsure between two levels, choose the lower.
...
**GTS-FILES-01 — Keep the fixed source-file section order.** — `medium` · judgment · [Guide: Source file structure]
...
**GTS-FILES-02 — Separate each present section with exactly one blank line.** — `low` · judgment · [Guide: Source file structure]
...
**GTS-IMPORTS-01 — Use ES module syntax for every import and export.** — `medium` · judgment · [Guide: Use modules not namespaces]
...
**GTS-IMPORTS-02 — Import repository code by relative path inside a package and by workspace name across packages.** — `medium` · judgment · [Guide: Import paths]
...
**GTS-IMPORTS-03 — Accept both extensionless and explicit `.ts` relative imports.** — `low` · judgment · [Repo: tsconfig.json `allowImportingTsExtensions`]
...
**GTS-IMPORTS-04 — Choose named versus namespace imports by readability.** — `low` · judgment · [Guide: Namespace versus named imports]
...
**GTS-IMPORTS-05 — Rename imports only to resolve collisions, generated names, or unclear names.** — `low` · judgment · [Guide: Renaming imports]
...
**GTS-IMPORTS-06 — Use side-effect imports only for libraries that must run on load.** — `low` · judgment · [Guide: Imports]
...
**GTS-EXPORTS-01 — Use named exports only; never `export default`.** — `medium` · mechanical · [Guide: Exports]
...
**GTS-EXPORTS-02 — Export only what other modules use.** — `medium` · judgment · [Guide: Export visibility]
...
**GTS-EXPORTS-03 — Do not create mutable exports (`export let`).** — `medium` · judgment · [Guide: Mutable exports]
...
**GTS-EXPORTS-04 — Do not build container classes to namespace related values.** — `medium` · judgment · [Guide: Container classes]
...
**GTS-VARS-01 — Declare with `const`, use `let` only when reassigned, and never use `var`.** — `high` · judgment · [Guide: Use const and let]
...
**GTS-VARS-02 — Declare one variable per declaration.** — `low` · judgment · [Guide: One variable per declaration]
...
**GTS-ARRAYS-01 — Do not use the `Array()` constructor or put non-numeric properties on arrays.** — `high` · judgment · [Guide: Do not use the `Array` constructor, Do not define properties on arrays]
...
**GTS-OBJECTS-01 — Do not use `Object()` or the primitive wrapper constructors.** — `high` · judgment · [Guide: Do not use the `Object` constructor, Wrapper objects for primitive types]
...
**GTS-SPREAD-01 — Spread only values of the kind being created.** — `high` · judgment · [Guide: Using spread syntax]
...
**GTS-SPREAD-02 — Use object spread for shallow copies only.** — `medium` · judgment · [Guide: Using spread syntax]
...
**GTS-DESTRUCTURE-01 — Destructure with intention: drop unused array slots and default inside the pattern.** — `low` · judgment · [Guide: Array destructuring]
...
**GTS-DESTRUCTURE-02 — Keep destructured parameters to one level of shorthand properties.** — `low` · judgment · [Guide: Object destructuring]
...
**GTS-OBJECTS-02 — Do not mix dict-style keys with identifier keys in one literal.** — `low` · judgment · [Guide: Computed property names, Computed properties]
...
**GTS-COLLECTIONS-01 — Pass an explicit initial value to `reduce`.** — `high` · judgment · [Not in guide]
...
**GTS-CLASSES-01 — Do not use constructor parameter properties.** — `medium` · judgment · [Repo: tsconfig.json `erasableSyntaxOnly`; Guide: Parameter properties is not applicable]
...
**GTS-CLASSES-02 — Initialize fields where they are declared.** — `medium` · judgment · [Guide: Field initializers]
...
**GTS-CLASSES-03 — Mark properties never reassigned after construction `readonly`.** — `medium` · judgment · [Guide: Use readonly]
...
**GTS-CLASSES-04 — Use `private`, never `#private` fields.** — `medium` · judgment · [Guide: No #private fields]
...
**GTS-CLASSES-05 — Respect visibility: omit `public`, never hide a cross-scope property, never bypass with `obj['x']`.** — `medium` · judgment · [Guide: Visibility, Properties used outside of class lexical scope]
...
**GTS-CLASSES-06 — Keep getters pure and accessors non-trivial.** — `medium` · judgment · [Guide: Getters and setters]
...
**GTS-CLASSES-07 — Do not use `this` in a static context or dispatch static methods dynamically.** — `high` · judgment · [Guide: Avoid static `this` references, Do not rely on dynamic dispatch]
...
**GTS-CLASSES-08 — Prefer module-local functions over private static methods.** — `low` · judgment · [Guide: Avoid private static methods]
...
**GTS-CLASSES-09 — Separate class methods and constructors with a single blank line.** — `low` · judgment · [Guide: Class method declarations, Constructors]
...
**GTS-CLASSES-10 — Call constructors with parentheses and delete empty or delegating constructors.** — `low` · judgment · [Guide: Constructors]
...
**GTS-CLASSES-11 — `toString` must always succeed and have no visible side effects.** — `medium` · judgment · [Guide: Overriding toString]
...
**GTS-CLASSES-12 — Do not modify prototypes or built-in objects.** — `high` · judgment · [Guide: Do not manipulate `prototype`s directly, Modifying builtin objects]
...
**GTS-CLASSES-13 — Do not introduce decorators; place documentation before any decorator.** — `low` · judgment · [Guide: Decorators, Place documentation prior to decorators]
...
**GTS-FUNCTIONS-01 — Prefer function declarations for named functions.** — `low` · judgment · [Guide: Prefer function declarations for named functions, Nested functions]
...
**GTS-FUNCTIONS-02 — Use a concise arrow body only when the return value is used.** — `low` · judgment · [Guide: Arrow function bodies]
...
**GTS-FUNCTIONS-03 — Wrap callbacks in an arrow when signatures differ.** — `high` · judgment · [Guide: Prefer passing arrow functions as callbacks]
...
**GTS-FUNCTIONS-04 — Avoid arrow-function class properties; bind with arrows at the call site.** — `medium` · judgment · [Guide: Arrow functions as properties, Rebinding `this`]
...
**GTS-FUNCTIONS-05 — Keep parameter initializers simple and side-effect free.** — `low` · judgment · [Guide: Parameter initializers]
...
**GTS-FUNCTIONS-06 — Use rest parameters, not `arguments`, and spread, not `apply`.** — `medium` · judgment · [Guide: Prefer rest and spread when appropriate]
...
**GTS-FUNCTIONS-07 — Use `this` only where it is well defined.** — `high` · judgment · [Guide: this]
...
**GTS-PRIMITIVES-01 — Do not use line continuations inside string or template literals.** — `low` · judgment · [Guide: No line continuations]
...
**GTS-PRIMITIVES-02 — Write numeric literals with lowercase `0x`/`0o`/`0b` and no stray leading zeros.** — `low` · judgment · [Guide: Number literals]
...
**GTS-PRIMITIVES-03 — Use literal Unicode characters; escape only non-printable or ambiguous ones.** — `low` · judgment · [Guide: Non-ASCII characters]
...
**GTS-PRIMITIVES-04 — Coerce with `String()`, `Boolean()`, template literals, or `!!` — never wrapper constructors.** — `high` · judgment · [Guide: Type coercion, Wrapper objects for primitive types]
...
**GTS-PRIMITIVES-05 — Parse numbers with `Number()` and check for `NaN`; never use unary `+` or bare `parseInt`/`parseFloat`.** — `high` · judgment · [Guide: Type coercion]
...
**GTS-CONTROL-01 — Brace every control-flow body.** — `low` · judgment · [Guide: Control flow statements and blocks]
...
**GTS-CONTROL-02 — Do not assign inside a condition unless parenthesized twice.** — `low` · judgment · [Guide: Assignment in control statements]
...
**GTS-CONTROL-03 — Every `switch` has a final `default`, and no non-empty case falls through.** — `high` · judgment · [Guide: Switch statements]
...
**GTS-CONTROL-04 — Iterate arrays with `for...of`; use `for...in` only on dict objects, guarded.** — `high` · judgment · [Guide: Iterating containers, Iterating objects]
...
**GTS-CONTROL-05 — Omit unnecessary grouping parentheses.** — `low` · judgment · [Guide: Grouping parentheses]
...
**GTS-CONTROL-06 — Keep `try` blocks focused on the throwing call.** — `medium` · judgment · [Guide: Keep try blocks focused]
...
**GTS-ERRORS-01 — Instantiate with `new Error(...)`, and throw or reject only `Error` subclasses.** — `high` · judgment · [Guide: Instantiate errors using `new`, Only throw errors]
...
**GTS-ERRORS-02 — Define a custom `Error` subclass when the native type lacks context.** — `low` · judgment · [Guide: Exception handling]
...
**GTS-ERRORS-03 — Assume caught values are `Error`; never swallow silently.** — `high` · judgment · [Guide: Catching and rethrowing, Empty catch blocks]
...
**GTS-ASYNC-01 — Do not leave promises floating.** — `high` · judgment · [Not in guide]
...
**GTS-TYPES-01 — Rely on inference and drop trivially inferred annotations.** — `low` · judgment · [Guide: Type inference]
...
**GTS-TYPES-02 — Add return type annotations where the return type is not obvious.** — `low` · judgment · [Guide: Return types]
...
**GTS-TYPES-03 — Avoid `any`; use `unknown` or a concrete type, and justify any suppression.** — `medium` · judgment · [Guide: `any` Type, Using `unknown` over `any`, Suppressing `any` lint warnings]
...
**GTS-TYPES-04 — Do not bake `|null` or `|undefined` into a type alias.** — `medium` · judgment · [Guide: Nullable/undefined type aliases]
...
**GTS-TYPES-05 — Prefer optional `?` over `|undefined` for fields and parameters, and initialize class fields.** — `medium` · judgment · [Guide: Prefer optional over `|undefined`]
...
**GTS-TYPES-06 — Define structural shapes as interfaces and annotate literals at their declaration.** — `medium` · judgment · [Guide: Use structural types]
...
**GTS-TYPES-07 — Prefer `interface` over an object type-literal alias.** — `low` · judgment · [Guide: Prefer interfaces over type literal aliases]
...
**GTS-TYPES-08 — Use `T[]`/`readonly T[]` for simple element types and `Array<T>` for complex ones; accept `readonly T[]` for arrays you do not mutate.** — `low` · judgment · [Guide: `Array<T>` Type, Use readonly]
...
**GTS-TYPES-09 — Give index-signature keys a meaningful label and prefer `Map`/`Record` for dictionaries.** — `low` · judgment · [Guide: Indexable types / index signatures]
...
**GTS-TYPES-10 — Prefer the simplest type construct that expresses the intent.** — `low` · judgment · [Guide: Mapped and conditional types]
...
**GTS-TYPES-11 — Use `Record` for statically known key sets and `Map` for dynamic keys.** — `low` · judgment · [Guide: Indexable types / index signatures, Mapped and conditional types]
...
**GTS-TYPES-12 — Use tuples instead of heavyweight pair interfaces.** — `low` · judgment · [Guide: Tuple types]
...
**GTS-TYPES-13 — Avoid creating return-type-only generic APIs and specify generics when consuming one.** — `medium` · judgment · [Guide: Return type only generics]
...
**GTS-TYPES-14 — Avoid type assertions; when one is necessary, state the reason and prefer `as` plus a runtime check.** — `medium` · judgment · [Guide: Type and non-nullability assertions, Type assertion syntax, Double assertions]
...
**GTS-TYPES-15 — Avoid the non-null assertion `!`; prove non-nullability at runtime or document why it cannot be null.** — `medium` · judgment · [Guide: Type and non-nullability assertions]
...
**GTS-TYPES-16 — Annotate object literals with `: Foo`, not `as Foo`.** — `medium` · judgment · [Guide: Type assertions and object literals]
...
**GTS-TYPES-17 — Do not introduce `enum` declarations.** — `medium` · judgment · [Repo: tsconfig.json `erasableSyntaxOnly`; Guide: Const enums is not applicable]
...
**GTS-NAMING-01 — Use the casing required by the identifier kind.** — `low` · judgment · [Guide: Rules by identifier type]
...
**GTS-NAMING-02 — Use descriptive names; do not abbreviate ambiguously or delete letters.** — `medium` · judgment · [Guide: Descriptive names]
...
**GTS-NAMING-03 — Treat acronyms as words.** — `low` · judgment · [Guide: Camel case]
...
**GTS-NAMING-04 — Do not use `_` as a prefix, suffix, or standalone identifier; avoid `$`.** — `low` · judgment · [Guide: `_` prefix/suffix, Dollar sign]
...
**GTS-NAMING-05 — Reserve `CONSTANT_CASE` for module-level and static readonly constants.** — `low` · judgment · [Guide: Constants]
...
**GTS-NAMING-06 — Make aliases mirror the source symbol and keep them `const`/`readonly`.** — `low` · judgment · [Guide: Aliases]
...
**GTS-DOCS-01 — Document exported symbols and non-obvious members; describe behavior, not types.** — `medium` · judgment · [Guide: Document all top-level exports of modules, Make comments that actually add information]
...
**GTS-DOCS-02 — Use TSDoc/JSDoc for documentation and `//` for implementation; no `/* */` blocks and no JSDoc type annotations.** — `low` · judgment · [Guide: JSDoc versus comments, Multi-line comments, JSDoc type annotations]
...
**GTS-DOCS-03 — Mark deprecated APIs with `@deprecated` and include migration guidance.** — `low` · judgment · [Guide: Deprecation]
