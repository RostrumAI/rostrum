...
This task replaces the phrase `rule set` and the related `ruleset` identifiers with terminology that tells readers what the code or document contains. It also decides whether Rostrum needs one object that groups every interface-version concern while only workflow interface v1 exists.

The current `InterfaceRuleSet` groups five different things:

- an exact interface-version token;
- the workflow document schema;
- the step-type registry;
- metadata fields excluded from publication digests;
- the ordered validation stages.

`RuleSetRegistry` then selects one of these objects from `interfaceVersion`. The phrase `rule set` does not tell a reader which of those responsibilities is relevant, and it is easy to confuse with validation rules alone. Epic 02 also needs to add execution-facing step contracts, which would make the term broader and less precise.

## Alternatives to assess

The implementation starts by comparing these alternatives against the current and planned callers.
...
| Split authoring and execution contracts | Separate the document-validation definition from the runtime step contract and select both through `interfaceVersion`. | Gives each object one purpose and prevents execution concerns from expanding a validator container. | Adds coordination between two versioned objects and may be premature while only v1 exists. |
| Validation profile | Rename the current object to a validation profile. | Familiar wording for a configured validation pipeline. | Incorrectly narrows an object that also controls digest metadata and step types. |

Start with `WorkflowInterfaceDefinition` and `WorkflowInterfaceRegistry` as the recommended names because the grouped values all describe one supported workflow interface version. Remove the grouped object instead if call-site inspection shows that callers use unrelated fields independently and direct exports from the v1 module are simpler. Do not use `ValidationProfile`; it hides the digest and step-type responsibilities.
...
Historical quotations can retain the original words when changing them would alter the source. Decision records must otherwise describe the current design rather than preserve obsolete implementation names.
...
Epic 02 extends the shared workflow contract before it builds execution. Leaving the current term in place would make every new runtime document harder to understand and would encourage more responsibilities to accumulate under an unclear abstraction.
...
- Every retained grouped object has one plain-language definition that lists what it contains and why those values must change together.
