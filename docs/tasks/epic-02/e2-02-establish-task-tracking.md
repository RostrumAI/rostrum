...
1. Delete it when the surrounding code already explains the behavior.
2. Replace it with the invariant or reason when the task ID is standing in for missing explanation.
...
Task IDs describe how work was organized at one point in time. They are not stable names for runtime behavior. Keeping them in code couples long-lived implementation details to a planning system that is already difficult to query and maintain.
