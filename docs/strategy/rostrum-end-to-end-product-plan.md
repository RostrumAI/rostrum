...
The [High-Level Build Blueprint](rostrum-high-level-build-blueprint.md) describes Rostrum's architecture and boundaries. This document defines what to build, the order in which it becomes useful, and the showcases that demonstrate the result.
...
| --- | --- | --- | --- |
| M1 | The shape of a workflow is defined and validatable | [Epic-01: Shape of a Workflow](../epics/epic-01-shape-of-a-workflow.md) | A human or AI saves incomplete workflow JSON as a draft, revises it from validation findings, publishes a valid revision, and retrieves its immutable version and digest. |
...
| M12 | The workflow CLI and showcase suite prove product breadth | [Epic-12: Workflow CLI and showcase suite](../epics/epic-12-showcase-suite.md) | The CLI exercises the stable workflow and Control API contracts, and every showcase below passes through the same public workflow, execution, policy, evidence, and client contracts on self-hosted Docker. |
