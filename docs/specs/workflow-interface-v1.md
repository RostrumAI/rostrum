...
The machine-readable schema ([workflow-interface-v1.schema.json](workflow-interface-v1.schema.json), JSON Schema 2020-12, emitted from the TypeBox source in `packages/workflow/src/schema.ts`) covers the document-shape rules that validation stage 2 enforces. Topology, conditional semantics, termination, data references, and lifecycle rules are enforced by the staged validator described in [Validation](#validation); JSON Schema cannot express them.
...
| 2. Document shape | Required fields, types, UUID v7 formats, `additionalProperties: false`, array bounds, `maxIterations >= 1`. Enforced by the [machine-readable schema](workflow-interface-v1.schema.json). Unknown fields are reported here | Interface version |
