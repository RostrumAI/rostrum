...
4. **Structured parallel branches (fan-out and fan-in):** When a workflow splits into parallel branches (a fan-out), all branches must rejoin at a single matching step (a fan-in) before the workflow can continue. Parallel branches cannot cross, end early, or contain nested conditionals. To branch after a join, the workflow must route from the fan-in step to a separate conditional step.
5. **Sequential bounded loops:** Loops process items in an array one at a time in collection order ($0, 1, \dots, n-1$). Iteration 1 starts only after iteration 0 finishes and commits its output. Each iteration returns an output object, and the loop step aggregates these into an ordered `results` array. If any iteration fails, the engine stops subsequent iterations.
...
8. **Complete failure reporting:** The engine does not pick a single primary failure when errors occur. When a step fails, the engine stops dispatching new steps, allows currently running steps to finish, and returns an array of all observed errors sorted consistently by step ID, iteration, error code, and document path.
9. **Efficient step readiness tracking:** The engine does not rescan the entire workflow graph after each step completes. Instead, each step tracks the count of upstream prerequisite steps it is waiting on. When a prerequisite finishes, the engine decrements that counter. When the counter reaches zero, the step becomes ready to run. This keeps scheduling fast and proportional to the steps and connections executed.
...
Epic 01 defines the shape and validation of a workflow document, but it does not specify how a workflow executes. E2-S1 defines the execution semantics for in-memory execution in the local daemon before the executor (E2-05) and Control API run operations (E2-06) are implemented.
...
Comparative research across workflow engines (AWS Step Functions, Temporal, Airflow, Argo Workflows, Prefect, Conductor, Camunda, Azure Durable Task, and n8n) is documented in [E2-S1 local execution semantics options](../../research/e2-s1-local-execution-semantics-options.md).

## Why these choices

The decision fixes a three-part architecture for local execution: an immutable compiled execution plan, a per-run transition reducer, and a read-only Control API projection. Publishing a workflow compiles it once into an execution plan that contains everything static about the graph: prerequisite counts, successor lists, conditional branch rules, and dependency links. Invoking a workflow creates the run's transition reducer, the only mutable state in local execution. The reducer advances step instances through their lifecycle, commits outputs as handlers finish, and runs inside the daemon independently of caller connections. Callers observe this activity only through the `currentSteps` and failure projections.

This separation maps directly onto the four requirements in Context. Compilation owns determinism because branch rules, priorities, and dependencies are validated once against an immutable graph. Per-run state isolation means concurrent runs never share mutable data, so daemon-owned execution survives caller disconnects. Read-only projections provide complete observability without giving callers a way to influence execution. Because every mutation happens at a reducer boundary, Epic 03 can wrap those boundaries in transactional checkpoints without changing successful execution behavior.

The following table summarizes each specific choice by aspect.

| Aspect | Choice | Reason |
| --- | --- | --- |
| Architecture | Immutable compiled plan with per-run transition reducer | Compiles static dependencies and parallel regions once, isolates mutable state to individual runs, and keeps graph execution fast and bounded. |
...
| Conditionals | Engine-evaluated by priority with mandatory `default` and explicit `next` targets | Keeps handlers simple and pure, removes custom routing logic from handlers, and ensures every conditional path resolves deterministically. |
| Parallel branches | Structured single-entry, single-exit (SESE) regions | Prevents race conditions, crossing branches, and dangling steps, ensuring parallel paths synchronize reliably at the join step. |
| Post-join routing (Q18) | Fan-in step cannot own a conditional; routing uses a separate successor step | Maintains clear boundaries for parallel regions and keeps the rule that parallel branches cannot connect directly to conditionals simple and verifiable. |
| Loops | Sequential bounded iterations with explicit output objects | Prevents infinite loops, guarantees predictable execution order, and produces a clean, structured array of iteration results. |
...
| Failure handling | Fail-fast dispatch with active handler drain and sorted failure lists | Halts new work immediately when an error occurs, lets in-flight tasks finish cleanly, and reports all observed errors in a predictable order. |
...
Run Lifecycle:
   ...
   [Invocation] ──(Valid)──> QUEUED ──(Start)──> RUNNING ──(Result Step Committed)──> SUCCEEDED
        │                                          │
    (Invalid)                               (Handler/Binding/Output Failure)
        │                                          │
        ▼                                          ▼
...
Step Instance Lifecycle:
   ...
   PENDING ──(Activated & Remaining Prerequisites = 0)──> READY ──(Worker Available)──> RUNNING ──(Valid Output)──> SUCCEEDED
                                                                                          │
...
| --- | --- | --- | --- | --- |
| `[None]` | Invocation request | The exact published workflow version exists, input keys match `workflow.inputs` exactly, and all required step handlers are registered | `queued` | Allocates a run ID, initializes execution tracking and prerequisite counters, and enqueues the `firstNode` step. |
...
| `running` | Step handler success | Handler output matches the exact step output schema and the step routes to a successor step | `running` | Commits step outputs, evaluates successors or conditionals, activates reached steps, decrements prerequisite counters, and updates `currentSteps`. |
...
| `running` | Failure observed | A step handler fails, output schema validation fails, input binding fails, or condition evaluation fails | `stopping` | Records the failure in the run's failure set, stops dispatching new steps, and allows in-flight handlers to finish. |
...
| Current state | Event / Trigger | Guard | Next state | Effect / Output |
| --- | --- | --- | --- | --- |
| `pending` | Predecessor step completes or `firstNode` initializes | The step is reached by an active execution path | `pending` | Marks the step as activated and sets its initial prerequisite counter. |
| `pending` | Upstream prerequisite completes | The step is activated and its remaining prerequisite count reaches 0 | `ready` | Adds the step to the ready queue; the step appears as `{ stepId, state: "ready" }` in `currentSteps`. |
...
| `running` | Handler returns error or throws exception | The error matches a structured failure or an unhandled exception occurs | `failed` | Adds the error to the run's failure set, removes the step from `currentSteps`, and moves the run to `stopping`. |
| `running` | Output validation fails | The returned output misses declared fields, includes undeclared fields, or has incorrect data types | `failed` | Adds a schema error to the run's failure set, removes the step from `currentSteps`, and moves the run to `stopping`. |
...
1. A step handler must return an explicit success object `{ kind: "success", outputs: Record<string, JSONValue> }` or failure object `{ kind: "failure", error: StructuredError }`.
2. Steps that produce no output values must return `{ kind: "success", outputs: {} }`.
...
Steps connected in a straight line advance sequentially. When step $A$ finishes, it commits its outputs and decrements step $B$'s prerequisite counter to zero, making step $B$ `ready`.
...
### 3. Structured parallel branches (fan-out and fan-in)

- **Single-entry, single-exit regions:** A fan-out step $D$ connects to multiple branch steps ($E, F$). All paths originating from $D$ must rejoin at exactly one matching fan-in step $G$.
- **Synchronized join:** The fan-in step $G$ lists all converging branch exit steps in its `dependencies`. Step $G$ becomes `ready` only after all converging branches finish successfully and decrement $G$'s prerequisite counter to zero.
- **Region boundaries:** Parallel branches cannot cross or merge before the matching fan-in step. Conditionals and terminal `result` steps are forbidden inside an open parallel region.
- **Post-join routing (Q18):** The matching fan-in step $G$ cannot own a conditional. If a workflow needs to branch after joining, step $G$ must route to a separate successor step that evaluates the conditional.
- **Independent dispatch:** All parallel branch roots enter the `ready` state in the same turn. The dispatcher executes them across available worker slots. Variations in worker speed or task completion order do not affect the joined outputs or the final result.
...
- Iteration $k+1$ starts only after iteration $k$ completes and commits its output. Only one iteration runs at a time.
- Each iteration executes its body graph and returns one explicit output object.
- When all iterations finish, the loop step returns an ordered array `results` containing the output objects from each iteration in collection order.
- If any iteration fails, the engine cancels subsequent iterations and fails the loop step.
...
2. **Fail-fast with active drain:** When the engine encounters the first failure:
   ...
   * Any additional failures returned by in-flight handlers are collected into the run's failure set.
...
1. **Immutable plan compilation:** Published workflows are compiled once into execution plans indexed by step position, containing static prerequisite counts, successor lists, and dependency links. Compilation takes $O(V + E)$ time and memory for $V$ steps and $E$ connections.
2. **Compact per-run state:** Mutable state for a run consists of bitsets and counters for activation status, remaining prerequisite counts, a FIFO ready queue, and committed output storage.
3. **Predictable execution cost:** For a run with $V$ steps and $E$ connections:
   ...
   * Updating prerequisite counters runs in exactly $E$ decrement operations.
...
## Required amendments to Epic 01 contracts

These execution semantics clarify and refine several provisional rules from Epic 01:

| Area | Epic 01 provisional rule | E2-S1 final execution rule | Required amendment |
| --- | --- | --- | --- |
...
| Parallel branches | Unstructured graph connections were permitted | Structured single-entry, single-exit regions are required | Update validator to enforce SESE regions, forbid nested conditionals/terminals in parallel paths, and forbid conditionals on fan-in steps. |
| Loops | Loop execution and result shapes were left open | Sequential iteration with one output object per iteration, aggregated into an ordered `results` array | Specify loop execution behavior and output shape in the reference schema. |
...
1. **Transactional checkpoints:** In Epic 03, the transition reducer boundaries defined here map directly to transactional database checkpoints. Committing step outputs, decrementing prerequisite counters, and updating `currentSteps` occur atomically.
...
3. [Handler] Step 'start' returns { kind: "success", outputs: { "greeting": "Hello, Ada" } }.
4. [Transition] 'start' SUCCEEDED. Outputs committed. Step 'transform' activated; remaining prerequisites = 0 -> READY.
5. [Handler] Step 'transform' runs with inputs { "text": "Hello, Ada" }, returns { kind: "success", outputs: { "upper": "HELLO, ADA" } }.
6. [Transition] 'transform' SUCCEEDED. Outputs committed. Step 'result' activated; remaining prerequisites = 0 -> READY.
...
### Scenario 3: Structured fan-out and fan-in

```text
Workflow: Split -> (BranchA, BranchB) -> Join -> Result

1. 'split' succeeds. Successors 'branchA' and 'branchB' activated.
2. Both 'branchA' (prerequisites = 0) and 'branchB' (prerequisites = 0) enter READY.
...
5. 'branchA' finishes, outputs committed. 'join' remaining prerequisites decremented (2 -> 1).
6. 'branchB' finishes, outputs committed. 'join' remaining prerequisites decremented (1 -> 0).
...
1. StepA handler returns { kind: "success", outputs: { "score": "high" } }.
