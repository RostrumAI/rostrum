...
- **Versioning methodology** — what constitutes an operational change (affecting execution behavior) vs a metadata change, which change types trigger a version bump, and how version changes affect actively running workflows — is under investigation in [E1-S4](../../tasks/epic-01/e1-s4-define-versioning-methodology.md). The current exact-match token contract for v1 is not altered by that investigation.
