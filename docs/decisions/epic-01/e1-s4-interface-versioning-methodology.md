...
Full option analysis with per-option pros and cons is recorded in the research doc [E1-S4 versioning methodology options](../../research/e1-s4-interface-versioning-methodology-options.md). Comparative platform research (Zapier, Temporal, n8n, Pipedream, Make, AWS Step Functions, GitHub Actions, Alchemer, Azure Logic Apps, Prefect) informed the choices; the recurring patterns were bind-on-start for in-flight runs, cutover-at-publish for future invocations, and deprecation deadlines bounding old-version support.
