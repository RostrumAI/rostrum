/** @fileoverview Declared-input contract tests for the controller builder. */

import { describe, expect, test } from "bun:test";
import { Type } from "typebox";
import { createControllerRegistrar, createServerApp } from "./app";
import { createControllerBuilder } from "./controller";
import { defineSchema } from "./schema";

/** The application context these controllers receive. */
...
/** Records what one handler received, so the controller contract is observable. */
interface Observed {
    runs: number;
    request?: Record<string, unknown>;
}

/** Builds an application whose single controller records its own request. */
...
describe("controller request contract", () => {
