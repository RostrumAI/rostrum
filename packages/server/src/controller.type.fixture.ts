/** @fileoverview Compile-time fixtures for the typed controller contract. */
...
interface FixtureContext {
    /** The services the application injects, so context access is type-checked. */
    readonly services: {
        /** Stands in for a domain operation a controller calls. */
...
const defineController = createControllerBuilder<FixtureContext, "system" | "workflows">();

/** Every declared input and every handler argument, inferred from one definition. */
export const withEverything = defineController({
    method: "POST",
    path: "/api/items/:id",
    ...
    handler: (request, response, context) => {
        ...
        const recordsCheck: Expect<Equal<typeof context.services.records, readonly string[]>> =
            true;
...
/** A controller that declares neither input: the request exposes only headers. */
export const withNothing = defineController({
    method: "GET",
    path: "/api/items",
    request: {},
...
/** A bodyless controller's request exposes headers only. */
export type BodylessRequestKeys = Expect<
    Equal<keyof ControllerRequest<undefined, undefined>, "headers">
>;

/** A context carries the application's own fields plus the reserved raw view, and nothing else. */
export type ContextKeys = Expect<
    Equal<keyof ControllerContext<FixtureContext>, "services" | "raw">
>;

/** A handler answers with a response, never with a plain value. */
export type HandlerAnswer = Expect<
    Equal<
        ReturnType<Controller<FixtureContext, undefined, undefined>>,
...
export type HandlerArity = Expect<
    Equal<Parameters<Controller<FixtureContext, undefined, undefined>>["length"], 3>
>;
...
export type ReservedRawBuilder = Expect<
    Equal<
        ReturnType<typeof createControllerBuilder<{ raw: string }, "system">>,
        {
...
export const withTwoParameters = defineController({
    method: "GET",
...
export type RequestKeys = Expect<
    Equal<
        keyof ControllerRequest<typeof bodySchema, typeof paramsSchema>,
