...
export class TerminationStage implements ValidationStage {
    ...
    run(context: ValidationContext): Finding[] {
        ...
        const visit = (stepId: string, path: Set<string>): void => {
            ...
            // A conditional step routes through its branches instead of
            // `successors`: walk each branch that continues, then the default.
            ...
            if (conditional) {
                ...
                // A branch or default with no `next` ends the workflow, so this
                // step counts as a leaf even though it has outgoing routes.
                const endsHere =
                    conditional.branches.some((branch) => !branch.next) ||
                    !conditional.default.next;
                if (!hasOutgoing || endsHere) {
                    leaves.add(stepId);
                }
                return;
            }
            // Plain steps follow their ordering edges; no edges means the
            // path ends here.
        ...
        for (const leafId of leaves) {
            ...
            if (isTerminal) {
                // Loop bodies may end at any step type because iteration outputs
                // are collected; only top-level terminals must be typed `result`.
            ...
            // Not a terminal step, so the path ended here only because every
            // route out was an end-workflow conditional branch; anything else
            // is a path that stops without ending the workflow.
