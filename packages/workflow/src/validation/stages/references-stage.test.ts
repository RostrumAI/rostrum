...
describe("ReferencesStage", () => {
  ...
  test("reports an unknown workflow input", () => {
    ...
    expect(finding?.details).toEqual({ stepId: task.id, ref: "inputs.missing", input: "missing" });
  });
  ...
  test("reports an undeclared output but resolves the reserved loop results output", () => {
    ...
    const loop = taskLoopStep(
      { collection: { ref: "inputs.f" }, maxIterations: 5, variable: "v", body: bodyTerminal.id },
  ...
  test("checks loop variable scope", () => {
    ...
    const loop = taskLoopStep(
      { collection: { ref: "inputs.f" }, maxIterations: 5, variable: "file", body: bodyStep.id },
      { successors: [after.id] },
    );
    const document = buildDocument({
