...
export class IdentityStage implements ValidationStage {
    ...
    private checkStep(
        ...
        if (firstIndex !== undefined && firstIndex !== index) {
            findings.push(
                context.findings.create({
                    ...
                    path: `/steps/${index}/id`,
                    relatedLocations: [
                        { path: `/steps/${firstIndex}/id`, message: "first occurrence" },
                    ],
            ...
            findings.push(
                context.findings.create({
                    ...
                    details: {
                        stepId: step.id,
                        received: step.type,
                        supported: this.stepTypes.types(),
                    },
                }),
            );
        } else {
        ...
        if (step.successors && step.conditional) {
            findings.push(
                this.mutuallyExclusive(context, step, index, "successors", "conditional"),
            );
        }
        ...
        if (
            step.conditional &&
            !(document.conditionals ?? []).some(
                (conditional) => conditional.id === step.conditional,
            )
        ) {
            findings.push(
                context.findings.create({
