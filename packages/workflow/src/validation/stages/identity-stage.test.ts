...
describe("IdentityStage", () => {
    ...
    test("reports duplicate conditional ids", () => {
        ...
        expect(
            findings.some(
                (finding) => finding.code === "workflow.identity.duplicate-conditional-id",
            ),
        ).toBe(true);
    });
    ...
    test("reports unknown targets for every reference field", () => {
        ...
        const document = buildDocument({
            inputs: {},
            steps: [
                taskStep({ successors: [missing], dependencies: [missing] }),
                looper,
                branching,
            ],
            conditionals: [
    ...
    test("reports an unregistered step type with supported types", () => {
        ...
        const findings = run(document);
        const finding = findings.find(
            (candidate) => candidate.code === "workflow.step.unknown-type",
        );
    ...
    test("validates config against the registered schema for known types", () => {
        ...
        const invalid = findings.filter(
            (finding) => finding.code === "workflow.step.invalid-config",
        );
    ...
    test("does not validate config of an unregistered type", () => {
        ...
        expect(findings.every((finding) => finding.code !== "workflow.step.invalid-config")).toBe(
            true,
        );
    });
});
