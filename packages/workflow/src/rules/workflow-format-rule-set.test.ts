...
import { V1_WORKFLOW_FORMAT_RULE_SET } from "./v1";
...
describe("WorkflowFormatRegistry", () => {
    test("selects a rule set by exact version match", () => {
        ...
        expect(registry.select("v1")).toBe(V1_WORKFLOW_FORMAT_RULE_SET);
    ...
    test("freezes registered rule sets", () => {
        ...
        registry.register(V1_WORKFLOW_FORMAT_RULE_SET);
        expect(Object.isFrozen(registry.select("v1"))).toBe(true);
    });

    test("keeps rule sets independent of registration order", () => {
        const later: WorkflowFormatRuleSet = { ...V1_WORKFLOW_FORMAT_RULE_SET, version: "v2" };
        const registry = new WorkflowFormatRegistry([later, V1_WORKFLOW_FORMAT_RULE_SET]);
