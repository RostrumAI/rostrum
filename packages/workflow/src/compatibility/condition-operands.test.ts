...
function buildConditional(condition: unknown): WorkflowConditional {
    return conditional({
        branches: [
            {
                label: "only",
...
function checkLeaf(schema: JsonSchema, leaf: { op: string; value?: unknown }) {
    const producer: SchemaProducer = { schema, root: schema };
    return checkConditionOperands(
        [buildConditional({ ref: REF, ...leaf })],
        createDeclaredSchemaCompiler(),
        () => producer,
...
describe("ordering operators", () => {
    ...
    test("need a number output and a number value", () => {
        ...
        for (const op of ["gt", "gte", "lt", "lte"]) {
            expect(checkLeaf({ type: "number" }, { op, value: 5 })).toEqual([]);
        }

        // A string output, or a string comparison value, can't be ordered.
        expect(checkLeaf({ type: "string" }, { op: "gt", value: 5 })).toEqual([
            ["operand-mismatch", LEAF],
        ]);
        expect(checkLeaf({ type: "number" }, { op: "lte", value: "5" })).toEqual([
            ["operand-mismatch", LEAF],
        ]);
    });
    ...
    test("an output the check can't compare is unprovable", () => {
        expect(checkLeaf({ not: { type: "string" } }, { op: "gt", value: 5 })).toEqual([
            ["unprovable", LEAF],
        ]);
    });
});
...
describe("contains", () => {
    ...
    test("needs a string output and value, or an array output", () => {
        ...
        expect(checkLeaf({ type: "array" }, { op: "contains", value: 1 })).toEqual([]);

        // A number output, or a string output searched for a number, can't match.
        expect(checkLeaf({ type: "number" }, { op: "contains", value: 1 })).toEqual([
            ["operand-mismatch", LEAF],
        ]);
...
describe("membership and equality", () => {
    ...
    test("in and notin", () => {
        ...
        expect(checkLeaf({ type: "number" }, { op: "in", value: ["a", 5] })).toEqual([]);

        // A non-array value, an empty array, and an array of unreachable elements fix the outcome.
        expect(checkLeaf({ type: "number" }, { op: "in", value: 5 })).toEqual([
            ["operand-mismatch", LEAF],
        ]);
        expect(checkLeaf({ type: "number" }, { op: "notin", value: [] })).toEqual([
            ["operand-mismatch", LEAF],
        ]);
        expect(checkLeaf({ type: "number" }, { op: "notin", value: ["a"] })).toEqual([
            ["operand-mismatch", LEAF],
        ]);
    });
    ...
    test("eq and neq", () => {
        ...
        expect(checkLeaf({ type: "number" }, { op: "eq", value: 5 })).toEqual([]);

        // A value outside it, or no value at all, fixes the outcome.
        expect(checkLeaf({ type: "number" }, { op: "neq", value: "five" })).toEqual([
            ["operand-mismatch", LEAF],
        ]);
        expect(checkLeaf({ type: "number" }, { op: "eq" })).toEqual([["operand-mismatch", LEAF]]);
    });

    // Proves an explicit null is a comparison value, distinct from an absent one.
    test("an explicit null value is compared", () => {
        expect(checkLeaf({ type: "null" }, { op: "eq", value: null })).toEqual([]);
        expect(checkLeaf({ type: "number" }, { op: "eq", value: null })).toEqual([
            ["operand-mismatch", LEAF],
        ]);
    });
});
...
describe("leaves the check doesn't judge", () => {
    ...
    test("truthy, falsy, and unknown operators", () => {
        for (const op of ["truthy", "falsy", "between"]) {
            expect(checkLeaf({ type: "number" }, { op })).toEqual([]);
        }
    });
    ...
    test("an unresolved reference is skipped", () => {
        const issues = checkConditionOperands(
            [buildConditional({ ref: REF, op: "gt", value: "x" })],
            createDeclaredSchemaCompiler(),
            () => undefined,
        );
...
describe("locating issues", () => {
    ...
    test("nested groups locate each leaf", () => {
        ...
        const routing = buildConditional({
            all: [
